...
listadoFelinos.forEach(felino => {
    ...
    <small>${felino.createdAt}</small>
    </div>`
})

listadoFelinos.forEach((item => {
    //capturando el div padre de todo
    ...
    //asignamos titulo al h2
    subtitulo.innerHTML = item.title
    //agregamos el atributo src y asignamos la imagen
    imagen.setAttribute("src", item.imgUrl)
    
    parrafo.innerText = item.description
    
    //agregamos lo creado al contenedor div del html
